...
struct ConfigReaderFactoryTests {
    ...
    func returnsFalseWhenConsulContainsJWKSKey() async throws {
        try await withApp(environment: .development) { app in
            ...
            let cachedConfigProvider = CachedConfigProvider(providerName: .consul, cachedValues: [jwksConfig.key: "eyJrZXlzIjpbXX0="])
            let result = ConfigReaderFactory.shouldLoadJWKS(
                jwksConfig: jwksConfig,
    ...
    func makeReturnsUsableReaderWithoutJWKSConfig() async throws {
        try await withApp { app in
            await ConfigReaderFactory.configureConfigReader(app: app, missingKeysKey: "missing-keys")
    ...
    func makeReturnsUsableReaderWithJWKSConfig() async throws {
        try await withApp { app in
            ...
            await ConfigReaderFactory.configureConfigReader(app: app, jwksConfig: jwksConfig, missingKeysKey: "missing-keys")
            #expect(app.configReader.string(forKey: "NONEXISTENT_KEY") == nil)
        }
    }
    ...
    func consulBeatsEnv() async throws {
        try await withApp(environment: .development) { app in
            ...
            await ConfigReaderFactory.configureConfigReader(app: app, keys: [key], missingKeysKey: "missing-keys")
